        ...
        <div class="item-list-wrap confirm-item-list-wrap">
          <div class="cart-item order-item">
            ...
            <ul class="cart-item-list">
              <li v-for="item in candidate_cartList" :key="item.productId">
...
export default {
  ...
  computed: {
    initalTotalPrice() {
      let sum = 0;
      this.candidate_cartList.forEach(item => {
    ...
    totalPrice() {
      return (
        this.initalTotalPrice + this.shippingFee + this.tax - this.discount
      );
    }
  },
  methods: {
    init() {
      this.axios.get("/api/users/cartList").then(res => {
        if (res.data.status == 0) {
          ...
          this.candidate_cartList = tempArr.filter(item => {
            return item.checked === true;
          });
          console.log(this.candidate_cartList);
        }
      });
    },
    makePayment() {
